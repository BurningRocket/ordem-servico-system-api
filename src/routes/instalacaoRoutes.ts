...
router.put('/update', async (req, res) => {
  try {
    const result = await instalacaoService.updateInstalacao(req.body);
    res.status(200).json(result);
  } catch (error: any) {
    console.log(error);
    res.status(500).json(error);
...
router.get('/findAllOpen', async (req, res) => {
  try {
    const result = await instalacaoService.getInstalacaosOpen();
    res.status(200).json(result);
  } catch (error: any) {
    console.log(error);
    res.status(500).json(error);
...
router.delete('/delete/:id', async (req, res) => {
  try {
    const result = await instalacaoService.deleteInstalacao(req.params.id);
    res.status(200).json(result);
  } catch (error: any) {
    console.log(error);
    res.status(500).json(error);
